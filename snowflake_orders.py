...
query_user_based_on_email = "SELECT NAME FROM SNOWFLAKE.ACCOUNT_USAGE.USERS where upper(email) = upper('%(email)s')"
grant_role_to_user = ["GRANT ROLE \"%(role)s\" to USER %(requesting_user)s"]
revoke_role_from_user = ["REVOKE ROLE \"%(role)s\" from USER %(requesting_user)s"]
default_collections_file = "snowflake_collections.csv"
use_user_map = True
