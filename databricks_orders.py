...
query_user_based_on_email = 'SELECT "%(email)s"'
grant_role_to_user = ["GRANT EXECUTE, SELECT, READ VOLUME, USE SCHEMA ON SCHEMA %(role)s to `%(requesting_user)s`"]
revoke_role_from_user = ["REVOKE EXECUTE, SELECT, READ VOLUME, USE SCHEMA ON SCHEMA %(role)s from `%(requesting_user)s`"]
default_collections_file = "databricks_collections.csv"

######################
current_filename = os.path.basename(__file__)

class Databricks_Session:
